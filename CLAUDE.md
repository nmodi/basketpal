...
- `AGENT_MODELS` — optional, comma-separated model slugs for the researcher (tool-calling) side of the agentic report loop (default in `openrouter_content_generator.py`)
- `WRITER_MODELS` — optional, comma-separated model slugs for the writer that turns the research dossier into prose (default in `openrouter_content_generator.py`)
- `JUDGE_MODEL` — optional, model slug for the shadow-mode recap judge (default in `openrouter_content_generator.py`)

### Agentic report generation

Pregame previews and postgame summaries are split into two roles. A researcher model (`AGENT_MODELS`, picked for tool reliability) runs a tool-calling loop (`_run_agent` in `openrouter_content_generator.py`): it gathers facts via data-fetching tools and ends with a terminal `finish_research` call (optionally passing angle notes), accumulating a dossier of full tool results. A writer model (`WRITER_MODELS`, picked for prose quality) then produces the report from that dossier in a single JSON call (`_write_report`). Every draft passes deterministic fact-checks (`_check_recap_facts` / `_check_preview_facts` — scores, player names, stat lines vs. the boxscore); failures re-prompt the writer with the problems and its rejected draft. The tool-call trace (including `finish_research`, `write_report`, and `fact_check` entries) is attached to the cached blob as `researchLog` and rendered by `ResearchLog.jsx`. If research fails for every model in `AGENT_MODELS`, or the writer for every model in `WRITER_MODELS`, generation falls back to the original single-shot prompt pipeline (no `researchLog`). After a recap is finalized, a shadow-mode LLM judge (`_shadow_judge_recap`, model `JUDGE_MODEL`) reviews it for narrative claims the deterministic checks can't reach; its findings are logged only and never block or alter the recap.
